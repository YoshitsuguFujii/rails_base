class RailsBase::UploadFileController < RailsBase::ApplicationController
  ...
  def create
    if params[:file_name].blank?
      ...
      upload_file = RailsBase::UploadFile.find_or_initialize_by(data_file_name: params[:file_name].original_filename)
  ...
  def destroy
    upload_file = RailsBase::UploadFile.find(params[:id])
    upload_file.destroy
  ...
  def load_instances
    @upload_files = RailsBase::UploadFile.all.order(id: :desc).page(params[:page]).per(PER_PAGE)
  end
end
